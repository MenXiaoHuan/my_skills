...
本 skill 用于把需求文档、PRD、技术方案、API 说明、原型描述等材料，转成**详细测试用例**。
默认交付物不是松散的测试点，而是一份可执行、可展开、可直接交给 QA 使用的 `detailed QA test cases`，通常落成真实 `.xmind` 文件。
...
> 该 skill 的定位是把需求材料转成详细测试用例树，而不是泛化测试顾问或代码工具箱。
