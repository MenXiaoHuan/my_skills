...
If parallel subtask tools such as `Agent` or `Task` are available, call those tools for Candidate A and Candidate B as independent analysis tracks. If no subtask mechanism is available, produce two separate internal drafts serially with the same candidate schema; do not collapse them into one blended draft.
...
- final tree must use the frozen target top-level module set unless a source-backed exception is recorded in internal notes
- case count must stay within the per-module budget unless the reference coverage floor requires a documented variance
