...
- `interaction-testcase-generator` is only the skill name and trigger term
...
- make every detailed case include `preconditions`, `steps`, and expected results
- use `No special preconditions` when a case needs no setup
- make every step include an expected result
...
Use this contract when no reliable file attachment is available, when running under runtime evaluation, or when the platform expects a text-only final answer.

Always:
- return exactly one fenced ```json block containing the normalized case tree
- make the JSON top-level object contain `groups`
- make each case an object with `title`, `priority`, `preconditions`, and `steps`
- make each case title start with `[P0]`, `[P1]`, `[P2]`, or `[P3]`
- make every step include `action` and `expected`
- keep Candidate A, Candidate B, the coverage ledger, and adjudication notes internal

Do not:
- include prose before or after that JSON block
- start the final response with exploration promises such as “I will inspect files”, “I will create cases”, or “I will generate XMind”
- return only summaries, coverage lists, statistics, or promises; summaries, coverage lists, statistics, or promises are not valid deliverables
- output scripts, commands, or XMind build instructions instead of the normalized JSON case tree
- expose intermediate build-file paths unless the user explicitly asks for debugging artifacts
...
- case: `title`, `priority`, `preconditions`, recommended `note`, optional `description`, `steps`
- step: `action`, `expected`, optional `note`

Structural completeness:
- `preconditions` is required for every detailed case; use `No special preconditions` when there is no setup
- `steps` must be a non-empty list for every detailed case
...
- cover storage, cache, message, callback, downstream dependency, idempotency, retry, authorization, or rollback effects when the technical document makes them test-relevant
- do not mechanically list every API; include only APIs directly tied to the case verification intent
- do not turn every case into a pure API test; keep the user-facing or business action as the primary step and attach API checks as observable verification
